...
#    SEZIONE 2 -- ANALYSIS (we upload the .RData and produce all the output)
#
...
# the file groupXX_data.RData. It is executed just if  the .RData doesn't exist.


if (!file.exists("groupXX_data.RData")) {
  
  cat("=== DATA PREPARATION: creation groupXX_data.RData ===\n")
  DATA_DIR <- "data"
  ...
  cat("  JKP factors: ", nrow(jkp_factors), "mesi,",
  ...
  # Pivotta: a column for each pf (1 = low, 3 = high)
  ...
  #     Warning: values are in % -> let's devide per 100.
  ...
  cat("  q5 factors:", nrow(q5_dt), "mesi,",
  ...
  #     Values in % -> let's divide per 100.
  ...
  for (ds_name in names(ff_datasets)) {
    ...
    cat("  Downloading", ds_name, "da Ken French...\n")
  ...
  bbg_file <- file.path(DATA_DIR, "Copia di SPX.xlsx")
  bbg_sheets <- list(
    spx       = "SPX",        # S&P 500 price only (1962+)
  ...
  for (col_name in names(bbg_sheets)) {
    ...
    if (col_name == "hy_oas") {
      ...
      # Monthly aritmetic return: (P_t / P_{t-1}) - 1
  ...
  # Are levels, not returns. We use the monthly average (already computed (from lorenzo)).
  ...
  save(master, file = "groupXX_data.RData")
  cat("\n  >>> Salvato: groupXX_data.RData <<<\n\n")
...
# Max drawdown (from return serie)
...
summary_table <- data.table(
  ...
  N_mesi     = sapply(stat_cols, function(c) sum(!is.na(master[[c]]))),
...
cat("\n\nTabella 2 -- Correlation matrix (pairwise complete)\n\n")
...
p_roll_cor <- ggplot(data.table(date = index(roll_cor_zf),
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  labs(title = paste0("Rolling Correlation Z-Score vs F-Score (", ROLL_W, " mesi)"),
...
overlay_dt <- merge(
  roll_sr_long[Factor == "Z-Score", .(date, z_sharpe = value)],
  master[, .(date, hy_oas, recession)],
  by = "date", all.x = TRUE
)
overlay_dt <- overlay_dt[!is.na(z_sharpe)]

# HY OAS just where available
p_regime <- ggplot(overlay_dt, aes(x = date)) +
  ...
  # Sharpe
  geom_line(aes(y = z_sharpe, color = "Z-Score Rolling Sharpe"), linewidth = 0.6) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  # HY OAS on secondary axis
  ...
  labs(title = "Z-Score: Rolling Sharpe vs HY OAS and Recessions",
       subtitle = "Grey bands = recessions NBER. High HY OAS = distress in credit",
       x = NULL, y = "Sharpe / OAS (scaled)") +
  coord_cartesian(xlim = range(overlay_dt[!is.na(z_sharpe)]$date)) +
  theme_arm
...
cat("\n====================================================\n")
cat("  SEZIONE 5 -- ANALISI PRE/POST PUBBLICAZIONE\n")
cat("====================================================\n")
cat("  Framework: McLean & Pontiff (2016)\n")
cat("  Z-score: Altman (1968). F-score: Piotroski (2000), campione 1976-1996.\n\n")
...
pub_analysis <- function(col, pub_year, sample_end_year, label) {
  ...
  # Se disponibile anche il sample-end del paper originale
...
cat("Tabella 3 -- Performance prima e dopo la pubblicazione\n")
cat("NB: per Z-Score il periodo pre-pub e' breve (dati partono da 1962).\n\n")
...
cat("\n====================================================\n")
cat("  SEZIONE 6 -- SPANNING REGRESSIONS\n")
cat("====================================================\n\n")
...
cat("Tabel 4 -- Spanning Regressions: Alpha and t-stat NW\n")
...
# --- 6c. Internal Spanning: f_score ~ z_score (e viceversa) ---
...
# SECTION 7 -- SUPPLEMENTAR ANALYSIS ----


cat("\n====================================================\n")
cat("  SECTION 7 -- Supplementar analysis\n")
cat("====================================================\n\n")
...
cat("\nTabella 6 -- Factor correlation vs benchmark and asset class\n\n")
print(round(bench_cor, 3))

# --- 7c. Plot benchmark comparison ---

# Using common sample at SPXT (from 1989)
bench_start <- as.yearmon("Sep 1989")
...
cat("============================================================\n")
cat("  Graph saved in : plots/\n")
cat("  Table printed in console (we need to copy them in the slides.\n")
cat("\n  Grafici prodotti:\n")
cat("    fig1_cumulative_returns.pdf\n")
cat("    fig2a_legs_zscore.pdf\n")
cat("    fig2b_legs_fscore.pdf\n")
cat("    fig3_rolling_sharpe.pdf\n")
cat("    fig4a_rolling_beta.pdf\n")
cat("    fig4b_rolling_alpha.pdf\n")
cat("    fig5_rolling_correlation.pdf\n")
cat("    fig6_regime_overlay.pdf\n")
cat("    fig7_benchmark_comparison.pdf\n")
cat("\n  Table computed:\n")
cat("    T1: Summary Statistics\n")
cat("    T2: Correlation matrix\n")
cat("    T3: Pre/Post publication\n")
cat("    T4: Spanning regressions\n")
cat("    T5: Benchmark comparison\n")
cat("    T6: Correlation vs asset class\n")
cat("============================================================\n")